...
- [x] 9.1 Docker コンテナ内で両モードのプレイ・クリア・ゲームオーバーを確認
- [x] 9.2 虫食いモードで隠す数がネットワークペイロードに漏れていないことを確認
- [x] 9.3 改ざん（不正な `(num1,num2,blank)` 組）が検証で拒否されることを確認
- [x] 9.4 ランキングがモード別に分離表示され、二重登録防止が機能することを確認
- [x] 9.5 既存の通常モードのスコア・挙動が後方互換であることを確認
