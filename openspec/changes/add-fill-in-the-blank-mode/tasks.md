...
- [ ] 6.1 `index.astro` の単一開始ボタンを「通常モード」「虫食いモード」の2ボタンに変更
- [ ] 6.2 `startNewGame(stage, mode)` でモードを受け取り、`/api/stages/start` に `mode` を送信
- [ ] 6.3 ルール説明文に虫食いモードの説明を追記

## 7. フロントエンド: ゲームプレイのモード対応（game.js / game.astro）

- [ ] 7.1 `gameStore` にモード状態を保持し、`more` 取得・登録・ランキング遷移まで引き継ぐ
- [ ] 7.2 `game.astro` の問題表示を `blank` で出し分け（`□ + b = c` / `a + □ = c` / 通常）し、`□` を強調表示
- [ ] 7.3 回答判定（`submit` 内）をモード対応に（虫食いは隠された項を正解とし、`sum` と見える項から復元）
- [ ] 7.4 `answersLog` に `blank` を含め、虫食いでも `num1`/`num2` を復元して送信
- [ ] 7.5 必殺技ゲージ加算が両モードで `10 + ((num1+num2)%16)` のまま一致することを確認

## 8. フロントエンド: ランキング画面のモード別表示

- [ ] 8.1 `ranking.astro` にモード切替（タブ／クエリ）を実装し、選択モードのトップ10を表示
- [ ] 8.2 結果遷移（`?r=<score_id>`）時に当該スコアのモードで順位＋前後表示を行う
- [ ] 8.3 ゲーム終了→登録→ランキング遷移時にモードを受け渡す
